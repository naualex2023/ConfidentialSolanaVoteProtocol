import { PublicKey, Transaction, SystemProgram, sendAndConfirmTransaction, TransactionInstruction } from '@solana/web3.js';
import { ArciumVoteClient } from './arciumClient';
...
export class CSVPVoteClient {
    ...
    programId;
    arciumClient;
    constructor(connection, programId, arciumCluster, wallet) {
     ...
     */
    async initializeElection(creator, electionId, title, description, startTime, endTime) {
     ...
     */
    async registerVoters(creator, electionPda, voterHashes, chunkIndex = 0) {
     ...
     */
    async castVote(voter, electionPda, voterHash, candidateId, electionId) {
        ...
        const [voterChunkPda] = await PublicKey.findProgramAddress([Buffer.from('voter_chunk'), electionPda.toBuffer(), Buffer.from('0')], this.programId);
        const [receiptChunkPda] = await PublicKey.findProgramAddress([Buffer.from('receipt_chunk'), electionPda.toBuffer(), Buffer.from('0')], this.programId);
        const [ballotChunkPda] = await PublicKey.findProgramAddress([Buffer.from('ballot_chunk'), electionPda.toBuffer(), Buffer.from('0')], this.programId);
        // === PDA для нулификатора ===
        const [nullifierPda] = await PublicKey.findProgramAddress([Buffer.from('election'), electionPda.toBuffer(), Buffer.from('nullifier'), Buffer.from(nullifier)], this.programId);
     ...
     */
    async tallyElection(electionPda) {
        console.log('Starting election tally with Arcium MPC...');
        // Получаем все зашифрованные бюллетени из блокчейна
        const encryptedBallots = await this.arciumClient.getEncryptedBallotsFromChain(electionPda, this.programId);
    ...
    async computeVoterHash(biometricData, globalSalt) {
        const data = new Uint8Array([...globalSalt, ...biometricData]);
        return this.hashData(data);
    }
    async computeReceiptId(receiptSeed, electionPda) {
        const data = new Uint8Array([...receiptSeed, ...electionPda.toBytes()]);
        return this.hashData(data);
    }
    async computeNullifier(voterHash, electionPda) {
     ...
     */
    async verifyVoteReceipt(electionPda, receiptId) {
        let chunkIndex = 0;
        while (true) {
            try {
...
//# sourceMappingURL=voteClient.js.map